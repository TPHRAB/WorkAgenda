const { getDBConnection, involvedInProject,
        currentDate, getLastInsertedId } = require('./utils');
 ...
 */
const isOwner = async (db, username, pid) => {
 ...
 * @param {number} pid - project id
 */
const createConnection = async (db, username, pid) => {
...
module.exports = {
   ...
   * @param {string} username
   * @param {number} pid - project id
   * @param {string} newUser - new user's username
   * @returns {Promise} resolved if success
   */
  addUserToProject: async (username, pid, newUser) => {
    let db = await getDBConnection();

    await isOwner(db, username, pid);

    await createConnection(db, newUser, pid);
    await db.close();
  },

  /**
   * Get users involved in the proejct
   * @param {string} username
   * @param {number} pid
   * @returns {Promise} [{username, first_name, last_name}, ...]
   */
  getProjectUsers: async (username, pid) => {
    let db = await getDBConnection();

    await involvedInProject(db, username, pid);
    let query = 'SELECT u.username, u.first_name, u.last_name, \
                        u.last_active_date \
                 FROM users as u, user_projects as up \
                 WHERE up.pid = ? AND up.username = u.username';
   ...
   * @param {string} username - user doing the request
   * @param {number} pid - project id
   * @param {string} userToRemove - user to be removed from the project
   */
  removeProjectUser: async (username, pid, userToRemove) => {
    let db = await getDBConnection();

    await isOwner(db, username, pid);

    if (userToRemove === username)
      await Promise.reject('You cannot remove yourself from the project');

    let statement = 'DELETE FROM user_projects WHERE pid = ? AND username = ?';
    await db.run(statement, [pid, userToRemove]);
    await db.close();
  },

  /**
   * Get setting information for the project
   * @param {string} username
   * @param {number} pid - project id
   * @returns {Promise} {name, owner, status, start, end}
   */
  getProjectSettings: async (username, pid) => {
    let db = await getDBConnection();

    await isOwner(db, username, pid);

    let query = 'SELECT name, owner, status, start_date, end_date \
                 FROM projects \
                 WHERE pid = ?';
    let result = await db.get(query, [pid]);
    await db.close();
