...
module.exports = {
  ...
  getBugInfo: async (username, bid) => {
    ...
    // get bug's comments
    query = 'SELECT cid, creator, comment, created_date \
             FROM comments \
             WHERE bid = ? \
  ...
  deleteBug: async (username, bid) => {
    ...
    // delete bug
    statement = 'DELETE FROM bugs WHERE bid = ?';
    await db.run(statement, [bid]);

    // delete comment
    statement = 'DELETE FROM comments WHERE bid = ?';
    await db.run(statement, [bid]);
  }
};